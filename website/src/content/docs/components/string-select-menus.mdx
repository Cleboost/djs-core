...
<Note>
  Files in `src/components/selects/string/` are auto-registered. `src/components/selects/string/pick-color.ts` gets `customId` `string.pick-color`.
</Note>
...
String select menus in djs-core are created using the `StringSelectMenu` class.
## Basic Select Menu
