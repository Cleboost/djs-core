...
<Steps>
  ...
  <Step title="Add scripts to package.json">
    ...
    <ResponseField name="dev" type="script">
      Runs the bot in development mode with hot reload for handlers under `src/` (restart manually after `djs.config.ts` changes)
    </ResponseField>
    ...
    <ResponseField name="check" type="script">
      Lints `src/` for djs-core anti-patterns (`djs-core check`)
    </ResponseField>
    <ResponseField name="test" type="script">
      Runs colocated `bun:test` suites via `djs-core test`
    </ResponseField>
  </Step>
