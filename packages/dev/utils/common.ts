...
export async function runBot(projectPath: string) {
    ...
    if (!config || typeof config !== 'object' || !('token' in config) || !('servers' in config)) {
      throw new Error('Config is not a valid config. It must have "token" and "servers" properties.')
    }
    ...
    commands.push(...await scanCommands(path.join(root, 'interactions', 'commands'), '', fileRouteMap))
    console.log(pc.green('✓') + `  Loaded ${pc.bold(commands.length)} commands`)

    buttons.push(...await scanButtons(path.join(root, 'interactions', 'buttons'), '', buttonFileRouteMap))
    ...
    client.once("clientReady", () => {
      ...
      console.log(pc.green('🚀 Bot is ready! ') + pc.dim(`Logged in as ${client.user?.tag}`))
    })
