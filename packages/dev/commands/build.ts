...
function buildGeneratedEntry(opts: {
  ...
  const { genDir, botRoot, commandsDir, buttonsDir, commandFiles, buttonFiles } = opts;
  ...
  const sortedCmds = commandRoutes.sort((a, b) => a.route.localeCompare(b.route));
  const sortedBtns = buttonRoutes.sort((a, b) => a.route.localeCompare(b.route));
...
export function registerBuildCommand(cli: CAC) {
  cli
    ...
    .option("-o, --outdir <outdir>", "Output directory (relative to project root)", { default: "dist" })
    ...
    .action(async (options: BuildOptions) => {
      ...
      if (buildType === "bun-external") {
        try {
          const botPackageJson = JSON.parse(await fs.readFile(botPackageJsonPath, "utf-8"));
          externalDeps = [
          ...
          console.warn(pc.yellow("⚠️  Could not read package.json, using default externals"));
      ...
      console.log(pc.green("✓") + `  Build success (${outputs.length} file${outputs.length === 1 ? "" : "s"}, ${sizeDisplay})`);
...
`;
        ...
        console.log(pc.green("✓") + `  Dockerfile created: ${pc.bold("Dockerfile")}`);
        ...
        try {
          ...
          await fs.writeFile(distPackageJsonPath, JSON.stringify(distPackageJson, null, 2), "utf8");
          console.log(pc.green("✓") + `  package.json created: ${pc.bold("package.json")}`);
