...
export function registerDevCommand(cli: CAC) {
  cli
    ...
    .action(async (options) => {
      ...
      const { client, root, fileRouteMap, buttonFileRouteMap } = await runBot(options.path)
      const commandsDir = path.join(root, 'interactions', 'commands')
      const buttonsDir = path.join(root, 'interactions', 'buttons')
      
      console.log(pc.dim(`\nWatching for changes in:\n - ${commandsDir}\n - ${buttonsDir}\n`))
      console.warn(pc.yellow('⚠️  Note: File deletions/renames might not be detected automatically on some systems.'))
      
      const commandsWatcher = watch(commandsDir, { recursive: true }, async (event, filename) => {
          ...
          if (!fileExists) {
              ...
              if (knownRoute) {
                  ...
                  fileRouteMap.delete(fullPath)
                   ...
                   console.log(pc.red('🗑️  Deleting route:') + ` ${pc.bold(probableRoute)} ` + pc.dim('(not in map)'))
          ...
          try {
              ...
              if (!command) {
                  console.warn(pc.yellow('⚠️  No default export found in ') + filename)
                  return
              }
      ...
      const buttonsWatcher = watch(buttonsDir, { recursive: true }, async (event, filename) => {
          if (!filename || !filename.endsWith('.ts')) return
          ...
          try {
              ...
              console.error(pc.red(`❌ Error reloading button ${filename}:`), error)
          }
      })
