...
type Entry = { type: string; color: (s: string) => string; route: string; file: string };
...
export function registerListCommand(cli: CAC) {
	cli
		.command("list", "List all detected commands, buttons, menus, events and cron tasks")
		...
		.action(async (options: { path: string }) => {
			...
			const contextsDir = path.join(root, PATH_ALIASES.interactions, "contexts");
			...
			const [cmdFiles, ctxFiles, btnFiles, selFiles, modFiles, evtFiles, cronFiles] =
				await Promise.all([
			...
			const total =
				cmdFiles.length + ctxFiles.length + btnFiles.length +
				selFiles.length + modFiles.length + evtFiles.length + cronFiles.length;
			...
			printSection("Commands", cmdFiles.map((f) => ({
				type: "cmd",
			...
			printSection("Context Menus", ctxFiles.map((f) => ({
				type: "ctx",
			...
			printSection("Buttons", btnFiles.map((f) => ({
				type: "btn",
			...
			printSection("Select Menus", selFiles.map((f) => ({
				type: "sel",
			...
			printSection("Modals", modFiles.map((f) => ({
				type: "modal",
			...
			printSection("Events", evtFiles.map((f) => ({
				type: "event",
			...
			printSection("Cron Tasks", cronFiles.map((f) => ({
				type: "cron",
			...
			const counts = [
				cmdFiles.length  && `${cmdFiles.length} command${cmdFiles.length > 1 ? "s" : ""}`,
				ctxFiles.length  && `${ctxFiles.length} context menu${ctxFiles.length > 1 ? "s" : ""}`,
				btnFiles.length  && `${btnFiles.length} button${btnFiles.length > 1 ? "s" : ""}`,
				selFiles.length  && `${selFiles.length} select menu${selFiles.length > 1 ? "s" : ""}`,
				modFiles.length  && `${modFiles.length} modal${modFiles.length > 1 ? "s" : ""}`,
				evtFiles.length  && `${evtFiles.length} event${evtFiles.length > 1 ? "s" : ""}`,
				cronFiles.length && `${cronFiles.length} cron task${cronFiles.length > 1 ? "s" : ""}`,
			].filter(Boolean);
