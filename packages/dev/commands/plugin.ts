...
async function runPostinstall(fullName: string, projectRoot: string) {
	try {
		let mainEntry = "";
		try {
			const pluginPkgPath = resolve(projectRoot, "node_modules", fullName, "package.json");
			const stats = await fs.stat(pluginPkgPath).catch(() => null);
			if (stats) {
				const pkg = JSON.parse(await fs.readFile(pluginPkgPath, "utf-8"));
				mainEntry = resolve(projectRoot, "node_modules", fullName, pkg.main || "dist/index.js");
			} else {
				const rootPluginPkgPath = resolve(projectRoot, "..", "..", "node_modules", fullName, "package.json");
				const pkg = JSON.parse(await fs.readFile(rootPluginPkgPath, "utf-8"));
				mainEntry = resolve(projectRoot, "..", "..", "node_modules", fullName, pkg.main || "dist/index.js");
			}
		} catch {
			const localPluginPkgPath = resolve(projectRoot, "..", "..", "plugins", fullName.replace("@djs-core/", ""), "package.json");
			const pkg = JSON.parse(await fs.readFile(localPluginPkgPath, "utf-8"));
			mainEntry = resolve(projectRoot, "..", "..", "plugins", fullName.replace("@djs-core/", ""), pkg.main || "dist/index.js");
		}
		...
		if (mainEntry) {
			...
			if (pluginObj && pluginObj.postinstall) {
				console.log(`${pc.cyan("ℹ")} Running postinstall for ${pc.bold(fullName)}...`);
...
export function registerPluginCommand(cli: CAC) {
	cli
		.command("plugin <action> <name>", "Manage bot plugins (install, postinstall)")
		.action(async (action: string, name: string) => {
			...
			if (action === "postinstall") {
				...
				if (!success) {
					console.log(pc.yellow(`⚠️  No postinstall hook found or failed for ${fullName}`));
				}
				process.exit(0);
			}
			...
			console.log(banner);
			console.log(`${pc.cyan("ℹ")} Installing plugin: ${pc.bold(fullName)}...\n`);
			...
			try {
				if (await fs.stat(configPath).catch(() => null)) {
					...
					if (!configContent.includes(fullName)) {
						console.log(`${pc.cyan("ℹ")} Adding ${pc.bold(fullName)} to ${pc.white("djs.config.ts")}...`);
						...
						const configKey = fullName.split("/").pop()?.replace("plugin-", "") || "";
				...
				console.warn(pc.yellow(`⚠️  Could not update djs.config.ts: ${(error as Error).message}`));
			}

			await runPostinstall(fullName, projectRoot);

			console.log(pc.dim("\nNext steps:"));
			console.log(pc.dim(`1. Configure the plugin in ${pc.white("pluginsConfig")} if needed (in djs.config.ts)\n`));
