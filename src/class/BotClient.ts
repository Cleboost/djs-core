...
export default class BotClient extends Client {
	...
	async start(token: unknown): Promise<void> {
		...
		if (!token) {
			this.logger.error("No token provided, please check your token (bot suhut down)");
			return process.exit(1);
		}
			...
			await fs.promises
				...
				.then(async (files) => {
					for (const file of files) {
						...
						const middleware = (await import(path.join(middlewaresPath, file))).default;
		...
		this.once("ready", async () => {
			...
			const subCommandList = ((await handlers.find((handler) => handler instanceof SubCommandHandler)?.getSubCommandGroupeDiscord()) ?? []).map((subCommand: SubCommandGroup) => subCommand.getDiscordCommand());
			const finalList = (commandsList ?? []).concat(subCommandList ?? []);

			this.application?.commands.set(finalList as unknown as Command[]).catch((e) => {
		...
		process.on('unhandledRejection', (reason: unknown) => {
			const errorRegex = /DiscordAPIError\[\d+\]: Invalid Form Body\s+data\.components\[\d+\]\.components\[\d+\]\.style\[BASE_TYPE_REQUIRED\]: This field is required/;
