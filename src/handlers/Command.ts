...
export default class CommandHandler extends Handler {
			...
			for (const categories of fs.readdirSync(commands)) {
				for (const command of fs.readdirSync(path.join(commands, categories)).filter((file) => file.endsWith(".js"))) {
					...
					if (this.collection.has(cmd.name)) {
						this.client.logger.warn(`The command ${underline(cmd.name)} is already loaded! Skipping...`);
						continue;
					}
	...
	async event() {
		this.client.on(Events.InteractionCreate, async (interaction: Interaction) => {
			...
			if (this.client.config?.logger?.logCmd) this.client.logger.info(`Command ${underline(interaction.commandName)} used  by ${interaction.user.username} (${interaction.user.id})`);
		});
	}
}
