...
export default class SelectMenuHandler extends Handler {
			...
			for (const selectType of fs.readdirSync(selectDir)) {
				for (const selectMenu of fs.readdirSync(path.join(selectDir, selectType))) {
					...
					if (!customID || typeof customID !== "string") {
						this.client.logger.error(`The select menu  ${underline(`${selectType}/${selectMenu}`)} has no customId!`);
						continue;
					}
					if (this.collection.has(customID)) {
						this.client.logger.warn(`The select menu  ${underline(`${selectType}:${selectMenuClass.getCustomId()}`)} is already loaded! Skipping...`);
						continue;
					}
	...
	async event() {
		this.client.on(Events.InteractionCreate, async (interaction: Interaction) => {
			...
			// }
			const select = this.collection.get(interaction.customId) as SelectMenu | undefined;
			
			if (!select) return interaction.reply({ content: "This select menu is not available", flags: [MessageFlags.Ephemeral]});
			select.execute(this.client, interaction);
			if (this.client.config?.logger?.logSelect) this.client.logger.info(`Select menu ${underline(interaction.customId)} used by ${interaction.user.username} (${interaction.user.id})`);
		});
	}
}
