...
export default class ButtonHandler extends Handler {
		...
		return new Promise<void>(async (resolve) => {
			...
			for (const button of fs.readdirSync(buttonsDir)) {
				if (fs.lstatSync(path.join(buttonsDir, button)).isDirectory()) {
					for (const buttonInSub of fs.readdirSync(path.join(buttonsDir, button))) {
						...
						if (this.collection.has(`${button}:${buttonClass.getCustomId()}`)) {
							this.client.logger.warn(`The button ${underline(`${button}:${buttonClass.getCustomId()}`)} is already loaded! Skipping...`);
							continue;
						}

						this.collection.set(`${button}:${buttonClass.getCustomId()}`, buttonClass);
					}
				}
				...
				if (!customID || typeof customID!== "string") {
					this.client.logger.error(`The button ${underline(button)} has no customId!`);
					continue;
				}

				if (this.collection.has(customID)) {
					this.client.logger.warn(`The button ${underline(customID)} is already loaded! Skipping...`);
					continue;
				}
	...
	async event() {
		this.client.on(Events.InteractionCreate, async (interaction: Interaction) => {
			...
			// }
			const button: Button | undefined = this.collection.get(interaction.customId) as Button | undefined;
			if (!button) return;
			button.execute(this.client, interaction);
			if (this.client.config?.logger?.logBtn) this.client.logger.info(`Button ${underline(interaction.customId)} used by ${interaction.user.username} (${interaction.user.id})`);
		});
	}
}
