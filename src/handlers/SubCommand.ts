        ...
        for (const command of fs
          ...
          for (const option of cmd.options as SlashCommandSubcommandBuilder[]) {
            if (!subCommandFiles.includes(option.name + ".js")) {
              return this.client.logger.warn(
                `Subcommand ${underline(option.name)} for ${underline(command)} is missing`,
              );
            }
            if (!option.name)
              return this.client.logger.error(
                `SubCommand ${option.name} for ${command} has no name`,
              );
            // eslint-disable-next-line @typescript-eslint/no-require-imports
            const subCmd = require(
              path.join(subCommandPath, option.name + ".js"),
            ).default;
            if (!(subCmd instanceof SubCommand))
              return this.client.logger.error(
                `Subcommand ${underline(option.name)} for ${underline(command)} is not a SubCommand`,
              );
    ...
    this.client.on(
      ...
      async (interaction: Interaction) => {
        if (interaction.isAutocomplete()) {
          ...
          const subCommand = this.collection.get(
